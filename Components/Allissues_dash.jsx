  ...
  useEffect(() => {
    const fetchCitizenFeed = async () => {
      ...
      try {
        ...
        const GEO_URL = `https://maps.googleapis.com/maps/api/geocode/json?latlng=${lat},${lng}&key=${GOOGLE_MAPS_API_KEY}`;
        const geoResponse = await fetch(GEO_URL);
        const geoData = await geoResponse.json();

        let context = "rural";
        if (
          geoData.results.some((result) =>
            result.types.some((t) =>
        ...
        // 🔹 Reverse geocode each issue’s coords (Google)
          ...
          try {
            ...
            if (geo.results && geo.results.length > 0) {
              const place =
                geo.results.find((r) =>
                  r.types.some((t) =>
                    ["neighborhood", "locality", "administrative_area_level_2"].includes(t)
                  )
                ) || geo.results[0];

              setIssues((prev) =>
                prev.map((i) =>
                  i._id === issue._id ? { ...i, address: place.formatted_address } : i
                )
              );
            }
          } catch (err) {
...
//   // 📍 fetch issues feed (runs on mount + refresh)
//   useEffect(() => {
//     const fetchCitizenFeed = async () => {
//       if (!location || !token || !user) return;
...
//     fetchCitizenFeed();
//   }, [location, token, user, refreshing]); // ✅ added refreshing
