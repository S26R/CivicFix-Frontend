...
const IssueDetails = () => {
  ...
  // State to hold issue details
  const [issue, setIssue] = useState(null);
  const [loading, setLoading] = useState(true);
  const [assignee, setAssignee] = useState("");
  const departments = [
    { id: "68d01532e76c4059f86a7722", name: "Roads & Infrastructure Department" },
    { id: "68d01534e76c4059f86a7725", name: "Street Lighting & Electricity Department" },
    { id: "68d01535e76c4059f86a7728", name: "Water Supply & Drainage Department Department" },
    { id: "68d01535e76c4059f86a772b", name: "Sanitation & Waste Management Department" },
    { id: "68d01536e76c4059f86a772e", name: "Public Safety & Transport Department" },
    { id: "68d01537e76c4059f86a7731", name: "Parks & Public Spaces Department" },
    { id: "68d01539e76c4059f86a7734", name: "Pollution Control Department" },
    { id: "68d0153ae76c4059f86a7737", name: "Animal Control Department" },
    { id: "68d0153be76c4059f86a773a", name: "General Department" },
  ];
  ...
  useEffect(() => {
    if (issue?.department) {
      // Find the department id from its name
      console.log("Finding department for:", issue.department);
      
      const found = departments.find((d) => d.name === issue.department);
      if (found) {
        console.log("Found department:", found);
        setAssignee(found.id); // pre-select the right id
      }
    }
  }, [issue]);
  ...
  const handleAssign = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/api/authority/issues/${id}/assignIssue`,
        {
          method: "PUT",
          headers: {
  ...
  return (
    <ScrollView className="flex-1 bg-white p-4">
      <View className="bg-orange-50 rounded-2xl border border-orange-200 shadow-lg p-4 mb-4">
        ...
        <Text className="text-sm text-gray-500 mb-2">
          Upvotes:{" "}
          <Text className="font-semibold">{issue.upvotes?.length || 0}</Text>
        </Text>
        ...
        {isAuthority && (
          <>
            ...
            <Picker
              selectedValue={assignee} // This will hold the 6-digit id
              onValueChange={(val) => setAssignee(val)}
              enabled={!issue.assignedDepartment}
              className={`border rounded-xl px-3 py-2 mb-3 text-orange-400 ${
            ...
            <TouchableOpacity
              ...
              className={`rounded-xl py-3 mb-2 ${
                issue.assignedDepartment ? "bg-gray-300" : "bg-orange-500"
              }`}
              activeOpacity={0.7}
            >
              <Text
